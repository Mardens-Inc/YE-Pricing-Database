...
pub async fn range(config: Data<DatabaseConfig>, request: HttpRequest) -> impl Responder {
	...
	match db_access::range(
		config, query, limit, std::cmp::max(1, page), sort, asc, employee, store, department, tag_number, from, to,
	) {
...
#[post("/")]
pub async fn test(config: Data<DatabaseConfig>, entry: Json<DatabaseInsertEntry>) -> HttpResponse {
	// Handle data access call and possible errors
