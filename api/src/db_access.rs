...
pub async fn export(
    ...
    {
        Ok(res) => {
            let employees: Vec<Employee> = match res.json().await {
                Ok(e) => e,
                Err(e) => return Err(e.to_string()),
            };
    ...
    let mut sql = "SELECT * FROM `years-end-inventory`".to_string();
    let mut conditions = Vec::new();
    let mut params: Vec<mysql::Value> = Vec::new();

    if let Some(q) = &query {
        conditions.push("description LIKE ?");
        params.push(mysql::Value::from(format!("%{}%", q)));
    }
    if let Some(emp) = employee {
        conditions.push("employee = ?");
        params.push(mysql::Value::from(emp));
    }
    if let Some(st) = store {
        conditions.push("store = ?");
        params.push(mysql::Value::from(st));
    }
    if let Some(dep) = department {
        conditions.push("department = ?");
        params.push(mysql::Value::from(dep));
    }
    ...
    let result: Vec<DatabaseRowEntry> = match connection.exec_map(
        sql.as_str(),
        params,
        |row: mysql::Row| -> DatabaseRowEntry {
            DatabaseRowEntry {
                id: row.get("id").unwrap_or_default(),
                tag_number: row.get("tag_number").unwrap_or_default(),
                store: row.get("store").unwrap_or_default(),
    ...
    let mut filename = "yeinv".to_string();

    if let Some(store) = store {
        if let Some(store_name) = stores.get(store as usize) {
            filename += format!("-{}", store_name).as_str();
        } else {
            return Err(format!("Invalid store index: {}", store));
        }
    }

    if let Some(department) = department {
        if let Some(department_name) = departments.get(department as usize) {
            filename += format!("-{}", department_name).as_str();
        } else {
            return Err(format!("Invalid department index: {}", department));
        }
    }

    if let Some(employee) = employee {
        let employee = employee as i32;
        if let Some(employee_data) = employees.get(&employee) {
            filename += format!("-{}", employee_data.first_name).as_str();
        } else {
            return Err(format!("Invalid employee ID: {}", employee));
        }
    }
